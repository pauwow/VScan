    ...
    with pd.ExcelWriter(output_file, engine="openpyxl") as writer:
        ...
        if "cashier" in df.columns and date_col:
            ...
            if not cashier_summary.empty:
                ...
                # Only breakdown rows, no cashier summary
                for _, row in cashier_summary.iterrows():
                    if "Cards List" in row and row["Cards List"]:
                        ...
                        raw_cashier_value = matches[0] if len(matches) > 0 else cashier_name

                        for card in cards:
                            new_row = row.copy()

                            # Rename Cards List -> Card Number
                            new_row["Card Number"] = card
                            if "Cards List" in new_row:
                                del new_row["Cards List"]

                            # Replace with card-specific stats
                            ...
                            # Override Register List → Register Used for this cashier-card pair
                            if "register_no" in df.columns:
                                reg_values = df[
                                    (df["cashier"] == raw_cashier_value) & (df["card_no"] == card)
                                ]["register_no"].dropna().unique()
                                if len(reg_values) > 0:
                                    new_row["Register Used"] = str(reg_values[0])
                                else:
                                    new_row["Register Used"] = "N/A"
                                if "Register List" in new_row:
                                    del new_row["Register List"]

                            # Remove distinct columns
                            for drop_col in [
                                "Distinct Branches", "Distinct Cashiers", "Distinct Registers", "Distinct Cards"
                            ]:
                                if drop_col in new_row:
                                    new_row.drop(labels=[drop_col], inplace=True, errors="ignore")
                ...
                # Ensure Register Used comes right after Branch List
                if "Branch List" in cols and "Register Used" in cols:
                    cols.remove("Register Used")
                    branch_idx = cols.index("Branch List")
                    cols.insert(branch_idx + 1, "Register Used")

                # Ensure Card Number comes before Sum of Transaction Total
                if "Card Number" in cols and "Sum of Transaction Total" in cols:
                    cols.remove("Card Number")
                    sum_idx = cols.index("Sum of Transaction Total")
                    cols.insert(sum_idx, "Card Number")

                expanded_df = expanded_df[cols]

                expanded_df.to_excel(writer, sheet_name="TopCashiers", index=False)

                # Apply highlighting (highlight first row for each cashier group)
                try:
                    wb = load_workbook(writer.path)
                    ws = wb["TopCashiers"]

                    cashier_fill = PatternFill(start_color="FFD700", end_color="FFD700", fill_type="solid")

                    last_cashier = None
                    for row_idx, row in enumerate(expanded_df.itertuples(index=False), start=2):
